...
def param_cfg_cpt():
    ...
    params = [(os.path.join(pytest.CODEB_PATH, "configs/inpainting/deepfillv1/deepfillv1_256x256_8x2_places.py"),
               utils.get_cpt(file_path="configs/inpainting/deepfillv1/deepfillv1_256x256_8x2_places.py",
...
def test_image_demo(cfg_cpt):
    ...
    file_path = os.path.join(pytest.CODEB_PATH, 'demo/inpainting_demo.py')
    # the cmd to be executed

    cmd = "python" + " " + file_path + " " \
          + cfg_cpt + " " \
          + os.path.join(pytest.CODEB_PATH, "tests/data/image/celeba_test.png") + " " \
          + os.path.join(pytest.CODEB_PATH, "tests/data/image/bbox_mask.png") + " " \
          + os.path.join(pytest.CODEB_PATH, "tests/data/pred/inpainting_celeba.png") + " " \
          + "--device=-1"
